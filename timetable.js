...
function computeWeekTypeFromDate(dateObj) {
    return window.TimetableCommon.computeWeekTypeFromDate(dateObj);
}
...
function getMappings(xmlDoc) {
    return window.TimetableCommon.buildMappings(xmlDoc, { includeWeeksdef: true });
}
...
function populateDepartmentSelect() {
    ...
    Object.values(mappings.teachers).forEach(teacher => {
        const departmentCode = window.TimetableCommon.extractDepartmentCode(teacher.short);
        if (departmentCode && departments[departmentCode]) {
            departmentSet.add(departmentCode);
        }
    });
...
function updateTeacherSelect() {
    ...
    const filtered = entries.filter(([, teacher]) => {
        ...
        const dept = window.TimetableCommon.extractDepartmentCode(teacher.short);
